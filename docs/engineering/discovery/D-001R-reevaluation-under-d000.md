# Discovery D-001R — Re-evaluation of D-001 Under D-000

**Method note on reproducibility:** for each candidate, "Hard Gate Results" report **Observation** (the constructed scenario or fact checked against the gate). "Strengths / Weaknesses" report **Interpretation**. "Recommendation" is the **Recommendation** proper, citing the specific gate(s) that drove it rather than an overall impression.
...
**Remaining Uncertainty:** Whether "habit-based" memory in small teams is a different phenomenon or just an informal representation of the same concern.
...
**Remaining Uncertainty:** Whether "organizational concern" is the right label if the phenomenon isn't organization-specific.

**Recommendation: Retain**, scope question logged as open.
...
**Remaining Uncertainty:** Whether "Judgment, broadened" is still usefully the same name — logged, not resolved (no new candidates introduced).

**Recommendation: Retain**, explicitly noting it will absorb content from Correction and Renewal.
...
**Remaining Uncertainty:** Whether "the channel" and "the willingness to notice/use it" should be split into two — tested here as one, per D-001's definition.
...
**Remaining Uncertainty:** Whether double-loop learning names a genuinely separate mechanism this test failed to detect, or a difficulty/frequency difference without a mechanism difference.

**Recommendation: Merge into Judgment** (moderate confidence — not a settled, closed question).
...
**Remaining Uncertainty:** D-000 as given has no formal category for "passes at one scale, doesn't exist as distinct at another."

**Recommendation: Reclassify** — not a top-level concern (fails a hard gate as tested), not a clean merge either (has a genuine, gate-1-confirmed independent failure mode). Best treated as a **scale-triggered specialization**: absent as a distinct category below some organizational size, first-class once that threshold is crossed.
...
**Remaining Uncertainty:** Whether drift-vulnerability is a property of this concern specifically, or evidence the model needs a mechanism (not currently named) for detecting silent drift over time.
...
**Remaining Uncertainty:** Whether "same mechanism, very different empirical difficulty" should be one concern with a documented scope distinction, or whether D-000's gates are simply not built to detect a concern whose distinctiveness is empirical/statistical rather than mechanistic.

**Recommendation: Merge into Judgment**, explicitly as an unbounded-scope property — flagged as the least clean merge in the set, given the structural-inertia counter-evidence.

---

## New Candidates

**Not introduced.** None of the four required conditions were jointly satisfied for any replacement. Accountability was reclassified, not rejected. Correction and Renewal were merged into an existing candidate, not replaced by a new one. "Trust/Cooperation," flagged in D-001A, was never itself run through D-000 in this exercise and is not introduced here.

---

## Meta-Evaluation

**Did D-000 improve the evaluation?**

- **More objective:** Yes. Reality-Testing's gate-1 test produced a different, better-supported conclusion than D-001A's informal suspicion, using the same underlying facts.
- **More repeatable:** Partially. Gate 1 and gate 4 still depend on the reviewer's skill at constructing scenarios; two reviewers could plausibly reach different verdicts on contested cases.
- **Easier to explain:** Yes — every recommendation cites a specific gate and constructed scenario.
- **Easier to challenge:** Yes, directly demonstrated — external-evidence signals surfaced genuine counter-evidence that complicated two hard-gate conclusions without reversing them.
- **Better at rejecting weak candidates:** Yes, with more precision — Accountability received a nuanced "Reclassify" verdict that D-001A's binary framing did not support.

**Limitations discovered while using D-000** (recorded as observations; D-000 itself not modified):

1. Gate 4 requires holding a full counterfactual model of the entire framework — the least mechanically repeatable of the four gates.
2. No provision for a candidate whose gate results are scale-conditional (Accountability).
3. No specification for weighing soft-signal counter-evidence against hard-gate conclusions when they point in different directions.
4. No formal category for "passes all gates, but only within a narrower scope than originally claimed."

---

## Decision Recommendation

**B. Retain with reservations.**

D-000 demonstrably improved on D-001A's ad hoc reasoning — it reversed one conclusion (Reality-Testing) using a constructed counterexample the earlier pass never attempted, sharpened two merge decisions into falsifiable claims, and produced a more precise, evidence-grounded verdict for a contested case (Accountability) than the earlier binary framing allowed. This satisfies the experiment's purpose: applying D-000 produced a demonstrably better evaluation than the original D-001A pass, on the same inputs.

This application also surfaced four concrete limitations in D-000 itself that were not present in D-000's own self-description and were only found by using it against real candidates. These are refinements, not invalidations. Per the constraint against redesigning D-000 in this exercise, they are logged as findings for a possible future, separately-authorized discovery into D-000's own limitations — not acted on here.
